...
def test_get_all_recipes_no_user(monkeypatch, client):
    # Stub the filter function to return a sample list
    monkeypatch.setattr('app.app.filter', lambda params, ui: ['r1', 'r2'])

    rv = client.get('/recipes?sortBy=alpha&timeCook=10&totalTime=20&prepTime=5&ready=true&ingredientsUsed=ing1&haveSome=false')
    assert rv.status_code == 200
    assert rv.get_json() == ['r1', 'r2']

# GET /recipes with user_id
def test_get_all_recipes_with_user(monkeypatch, client):
    # Stub user ingredients and filter
    monkeypatch.setattr('app.app.get_user_ingredients', lambda uid: {'ingredients': ['salt']})
    monkeypatch.setattr('app.app.filter', lambda params, ui: ['r3'])
...
def test_get_recipe_success(monkeypatch, client):
    monkeypatch.setattr('app.app.get_recipe_by_id', lambda rid: {'id': rid, 'name': 'Test'})
...
def test_get_recipe_not_found(monkeypatch, client):
    monkeypatch.setattr('app.app.get_recipe_by_id', lambda rid: None)
...
def test_get_all_ingredients(monkeypatch, client):
    monkeypatch.setattr('app.app.get_ingredients', lambda: ['salt', 'pepper'])

    rv = client.get('/ingredients')
    assert rv.status_code == 200
    assert rv.get_json() == ['salt', 'pepper']

# GET /user/<user_id>/ingredients success
def test_get_user_ingredients_success(monkeypatch, client):
    monkeypatch.setattr('app.app.get_user_ingredients', lambda uid: {'ingredients': ['ing']})

    rv = client.get('/user/u1/ingredients')
    assert rv.status_code == 200
    assert rv.get_json() == {'ingredients': ['ing']}

# GET /user/<user_id>/ingredients not found
def test_get_user_ingredients_not_found(monkeypatch, client):
    monkeypatch.setattr('app.app.get_user_ingredients', lambda uid: None)

    rv = client.get('/user/u1/ingredients')
    assert rv.status_code == 404

# PUT /user/<user_id>/ingredients success
def test_update_user_ingredients_success(monkeypatch, client):
    monkeypatch.setattr('app.app.update_user_ingredients', lambda uid, ings: True)

    rv = client.put('/user/u1/ingredients', json=['a', 'b'])
    assert rv.status_code == 200
    assert rv.get_json() == {'user_id': 'u1', 'ingredients': ['a', 'b']}

# PUT /user/<user_id>/ingredients bad request
...
# PUT /user/<user_id>/ingredients failure
def test_update_user_ingredients_failure(monkeypatch, client):
    monkeypatch.setattr('app.app.update_user_ingredients', lambda uid, ings: False)
...
def test_generate_shopping_list_success(monkeypatch, client):
    # Stub recipe and user data
    monkeypatch.setattr('app.app.get_recipe_by_id', lambda rid: {
        'name': 'R',
...
# GET /recipe/<id>/shopping-list recipe not found
